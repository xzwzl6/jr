...
//	window.location.href = 'http://www.zzly99.com/zzly_normal/jieri.html?name=' + name;
...
$(function() {
	...
	for(var v in festList) {
		//  	alert(festList[v].festName);
		//  	$('.festlist').append(`<div class="item" data-festname="${festList[v].festName}"><div class="text-l"><div class="text-l1">${festList[v].festName}</div><div class="text-l2">${festList[v].festDay}</div></div><div class="text-r">${festList[v].festNextTime}</div></div>`);

//		if(v < 1) {
//			$('.textbox .text1').text(v.festName);
//			getFestInfo(festList[v].festName, function(data) {
//				const result = data.data;
//				//              share.imgUrl = result.shareImg;
//				//              share.desc = result.shareTxt;
//				$('.head').css('backgroundImage', 'url(' + result.bgImg + ')');
//				$('.page').addClass('hidden');
//				$('.home').removeClass('hidden');
//			});
//			$('.head .text1').text(festList[v].festName);
//			$('.textbox .text2-l').text(festList[v].festDay);
//			$('.textbox .text2-r').text(festList[v].festNextTime);
//			//			$('.head').onClick = itemClick('冬至');
//			$(".head").each(function() {
//				$(this).click(function() {
//					itemClick(
//						festList[0].festName
...
//		} else {
$('.home').removeClass('hidden');
			var html = "<div  class='item'  onclick=itemClick('" +
				festList[v].festName + "') >" +
				"<div class='text-l'><div class='text-l1'>" +
				festList[v].festName + "</div>" +
				"<div class='text-l2'>" +
				festList[v].festDay + "</div></div>" +
				"<div class='text-r'>" +
				festList[v].festNextTime + "</div>" +
				"</div>";

			// 		$('.festlist').append(`<div class="item" data-festname="${festList[v].festName}"><div class="text-l"><div class="text-l1">${festList[v].festName}</div><div class="text-l2">${festList[v].festDay}</div></div><div class="text-r">${festList[v].festNextTime}</div></div>`);

			$('.festlist').append(html);
		}
//	}

	//  festList.forEach((v, k) => {
	//      if (k < 1) {
	//          $('.textbox .text1').text(v.festName);
	//          getFestInfo(v.festName, function (data) {
	//              const result = data.data;
	//              share.imgUrl = result.shareImg;
	//              share.desc = result.shareTxt;
	//
	//              $('.head').css('backgroundImage', 'url(' + result.bgImg + ')');
	//              $('.head').attr('data-festname', v.festName);
	//              $('.page').addClass('hidden');
	//              $('.home').removeClass('hidden');
	//          });
	//          $('.textbox .text2-l').text(v.festDay);
	//          $('.textbox .text2-r').text(v.festNextTime);
	//      } else {
	//          $('.festlist').append(`<div class="item" data-festname="${v.festName}"><div class="text-l"><div class="text-l1">${v.festName}</div><div class="text-l2">${v.festDay}</div></div><div class="text-r">${v.festNextTime}</div></div>`);
	//      }
	//  });

	//  $('.home .item,.home .head').on('click', function () {
	//      const name = encodeURIComponent($(this).data('festname'));
	//      const year = new Date().getFullYear();
	//      window.location.href = 'http://www.zzly99.com/zzly_normal/jieri.html?name=' + name + '&year=' + year;
	//  });
	//    api
	function getFestInfo(name, callback) {
		$.ajax({
			url: '//c.51wnl.com/contentapi/api4.4.0/GetTermOrFestival/GetInfo',
			dataType: "jsonp",
			jsonp: "callback",
			data: {
				name: name
